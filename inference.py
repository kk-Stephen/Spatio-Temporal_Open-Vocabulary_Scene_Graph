...
from semantic_map import create_video_scene_graph
from planning import generate_prompt
...
def main():
    ...
    parser.add_argument("--prompt_dir", type=str, required=True, default='full_prompt.txt')
    ...
    # full_prompt = generate_prompt(video_SG)
    ...
    with open(args.prompt_dir, 'r', encoding='utf-8') as f1, open(os.path.join(os.path.dirname(__file__), 'prompts', 'extra.txt'), 'r', encoding='utf-8') as f2:
        #prompt2 = f2.read()
        prompt1 = f1.read()
        # prompt = prompt1 + "\n" + prompt2
        prompt= prompt1
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    # "text": "descript what is happening from 6.0s to 7.5s"
                    "text": prompt
                },
            ],
        }
