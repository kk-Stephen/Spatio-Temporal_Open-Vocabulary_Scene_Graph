...
def main():
    ...
    # for index, row in tqdm(df.iloc[216:221].iterrows(), total=21, desc="Evaluating plans"):
    for index, row in tqdm(df.iterrows(), total=df.shape[0], desc="Evaluating plans"):
        ...
        if not os.path.exists(video_path):
            print(f"Warning: Skipping row {index} due to missing video file.")
            continue
    ...
    - Command absolute timestamp (UTC or epoch): 8.0
    - Known one-way latency from user to robot (seconds): 0.0
    ...
    1) Reconstruct what the user likely SAW at 8.0:
       - Prefer frames whose frame.observe_ts is closest to 8.0. If observe_ts is unavailable, approximate using frames with capture_ts closest to 8.0-0.0.
    ...
    - The selected instance must be consistent with the latency-aware intention at 8.0 and available now.
    - If not feasible, return [].
    
    - Final Output: Your entire response MUST be a single, valid JSON array of strings containing the action plan.

    [EXAMPLE OF A PERFECT OUTPUT]
    {{
    "action_plan": [
    "Pick cup (centroid:[0.006, -0.022, 0.127], size:[0.149, 0.081, 0.057])",
    "Place it next to the keyboard (ID 5)"
    ]
    }}

    Now, generate the plan for the given user command.
    YOUR RESPONSE MUST BE ONLY THE JSON OBJECT. DO NOT OUTPUT ANY OTHER TEXT, REASONING, or EXPLANATION.
        """
        ...
        evaluation_results.append({
            ...
            "predicted_plan": robot_plan,  # 用'|'连接，方便存储
